...
site_horario_floripa = 'http://tempo.cptec.inpe.br/sc/florianopolis'
...
def fases_lua2():
	...
	f = open('lua-datetime.csv').read()

	datas = [el.split('\t') for el in  f.split('\n')][1:]

	lista_media = []

	for el in datas:
		if datetime.strptime(el[0], '%Y-%m-%d %H:%M:%S') > dia:
...
def baixa_mar():
	os.chdir('\\\\172.20.0.45\\jornalismo novo\\# Robôs\Tempo\# script')
	global mares
	file_ = open('mares.txt')
	mares = [el.split('\t') for el in file_.read().split('\n')]
	mares_ = {el[0]:el[2] for el in mares}
	file_.close()

	amanha = datetime.now() + timedelta(days = 1) - timedelta(hours = 5)
	amanha_ = amanha.strftime('%Y-%m-%d')
...
def horario_sol(site_horario=site_horario_floripa, cidade=florianopolis_outros):
	...
	amanha = soup.find('div','previsao text-center').next_sibling.next_sibling
	horarios = amanha.find('div', 'd-flex flex-column nascer-por-sol')
	horarios_ = [el.text.replace('\n','') for el in  horarios.findAll('div')]
	nascente = horarios_[0]
	poente = horarios_[1]
